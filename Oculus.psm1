...
<#
 .Synopsis
  Provides basic network scanner functionality.

 .Description
  Provides basic network scanner functionality. Includes functions such as Host Discovery andd basic TCPConnection Scanning which is using three way handshake to discover open ports.
...
function Get-ConnectScan{
    ...
    begin {
        ...
        foreach($ip in $Target){
            if ($ip.IndexOf('/') -ne -1 ){ #CIDR
                $netadd,$prefix = $ip | foreach split /
                $IPs += (Get-IPNetwork -IPAddress $netadd -PrefixLength $prefix -ReturnAllIPs).AllIPs
            }
            elseif ($ip.IndexOf('-') -ne -1){ #RANGE
                $netadd = $ip.substring(0,12)
                [int]$r1,[int]$r2 = $ip.substring($ip.LastIndexOf('.') +1) | % split -
                for ($r1 ; $r1 -le $r2; $r1++) {
                    $IPs +=$($netadd) + $($r1)
                }
            }
    ...
    process {
        ...
        "`nScan report: `n"
        "$counter ports are in closed or filtered state.`n "
        Write-Output $FinalResult `n
        "Scanning done: " + $IPs.Length + " IP addresses scanned."
    }
...
function Get-HostDiscovery{
    ...
    process {
        ...
        "`n$counter host are down or filtered.`n"
        Write-Output $FinalResult `n
        "Scanning done: " + $IPs.Length + " IP addresses scanned."
    }
}