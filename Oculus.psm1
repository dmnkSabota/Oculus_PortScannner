...
function Get-TargetEnumeration{
    ...
    .DESCRIPTION
        Get-TargetEnumeration is function that creates simplified list of IP adresses from various input formats such as IP address range, IP address enumeration or IP address with CIDR notation. 
    .PARAMETER Target
    ...
    process {
        foreach($ip in $Target){
            ...
            if ($ip.IndexOf('/') -ne -1 ){#CIDR
                ...
                $iplist += (Get-IPNetwork -IPAddress $netadd -PrefixLength $prefix -ReturnAllIPs).AllIPs
            }
